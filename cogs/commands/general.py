...
Aliases""")
      ...
      for member in ctx.guild.members:
       ...
       # if member.status == discord.Status.idle:
        ...
        if member.bot:
          ...
          humans += 1
      embed = discord.Embed(title=ctx.guild.name,description="Here is **`%s`**'s member information" % (ctx.guild.name))
    ...
    @commands.hybrid_command(name="hack",
                             help="Hacks a user's account.",
                             usage="Hack <member>")
    @blacklist_check()
    @ignore_check()
    async def hack(self, ctx: commands.Context, member: discord.Member):
        addr = ".".join(str(random.randint(0, 255)) for _ in range(4))
        genders = ["Male","Female","Non-binary","Lesbian","Gay","Bisexual",]
        gender = random.choice(genders)
        embed = discord.Embed(
            title="**__HACKED !__**",
            description=f"**A dangerous hack on `{member.display_name}` has been successful. \n <:check:1089647181457272963> Details - \n IP : {addr} \nCOUNTRY CODE : +91 \nPHONE NUMBER : ||Phone number has been dmed to you||\nUSER ID - {member.id} \nGender : {gender}**",
          color=0x41eeee
    )
        embed.set_footer(
          text="Other detail has been dmed to you.",
          icon_url=ctx.author.avatar.url)
        await ctx.send(embed=embed)
    ...
    @commands.command(name="users", help="check users of Redox .")
    @blacklist_check()
    @ignore_check()
