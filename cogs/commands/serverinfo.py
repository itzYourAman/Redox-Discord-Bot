...
{guild.description}
        ...
        if guild.features:
            ...
            embed.add_field(
                name="**__Features__**",
                value='\n'.join([f"<a:tickkk:1130730211382661171>  : {feature}" for feature in dk]),
                inline=False
            )
...
Channels: <a:Hashtag:1136184948664057937> {len(guild.text_channels)} | <a:voicegoogle:1138834705622499339> {len(guild.voice_channels)} | <:stage:1139193434616377437> {len(guild.stage_channels)} 
{bye}
            """,
            inline=False
        )
...
Total Emoji: {len(guild.emojis)}/{uf}
        ...
        embed.add_field(
            ...
            value=f"""
Level: {guild.premium_tier} [<a:RH_red_diamond:1138848159574016131> {guild.premium_subscription_count} boosts] 
{byecom}
            """,
            inline=False
        )
