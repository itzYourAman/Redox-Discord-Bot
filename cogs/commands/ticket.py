...
async def configdata(guild: discord.Guild):
    ...
    if m_db is None:
        return None
    embed = discord.Embed(title=f"{m_db['name']} Ticket panel setup", color=0x2f3136).set_footer(text=guild.me.name, icon_url=guild.me.avatar.url)
...
class tickredel(discord.ui.View):
    ...
    async def _treopen(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if p is not None:
            ...
            message = f"{user.mention} Welcome Back! The ticket is reopened"
        embed = discord.Embed(description="You will be provided with support shortly\nTo close this ticket click the <:tickett:1209484988513517579> button.", color=0x2f3136).set_footer(text=f"{guild.me.name} Ticket System", icon_url=guild.me.avatar.url)
...
class ticketchannelpanel(discord.ui.View):
    ...
    @discord.ui.button(label="Close", emoji="<:tickett:1209484988513517579>", custom_id="close", style=discord.ButtonStyle.red)
    async def _ticketchannel(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        em = discord.Embed(title="<:tickett:1209484988513517579> Ticket Closed", description=f"{interaction.channel.mention} is closed by {str(interaction.user)}", color=0x2f3136).set_footer(text=f"{guild.me.name} Ticket System", icon_url=guild.me.avatar.url)
...
class editpanelview(discord.ui.View):
    ...
    async def support(self, interaction, button):
        ...
        for i in interaction.guild.roles:
            if not i.is_bot_managed() and not i.is_premium_subscriber():
                xd[i.name] = i.id
        if len(xd) == 0:
            await interaction.message.edit(embed = discord.Embed(title=f"{self.name} Ticket panel setup", description="There are no Roles in the Server", color=0x2f3136).set_footer(text=f"{self.bot.user.name} Ticket", icon_url=self.bot.user.avatar.url))
            await asyncio.sleep(10)
        else:
            x = rolemenuview(self.ctx, "supportrole", "Select the Support Role")
            embed = discord.Embed(title=f"{self.name} Ticket panel setup", description="Select a role which should be allowed to view the ticket channels\nIf a role is not listed below just start typing it's name in select menu box it will be shown as a option", color=0x2f3136).set_footer(text=f"{self.bot.user.name} Ticket", icon_url=self.bot.user.avatar.url)
    ...
    async def pingrole(self, interaction, button):
        ...
        for i in interaction.guild.roles:
            if not i.is_bot_managed() and not i.is_premium_subscriber():
                xd[i.name] = i.id
        if len(xd) == 0:
            await interaction.edit_original_response(embed = discord.Embed(title=f"{self.name} Ticket panel setup", description="There are no Roles in the Server", color=0x2f3136).set_footer(text=f"{self.bot.user.name} Ticket", icon_url=self.bot.user.avatar.url))
            await asyncio.sleep(10)
        else:
    ...
    async def open(self, interaction, button):
        ...
        for i in interaction.guild.categories:
                xd[i.name] = i.id
        if len(xd) == 0:
            await interaction.edit_original_response(embed = discord.Embed(title=f"{self.name} Ticket panel setup", description="There are no category in the Server", color=0x2f3136).set_footer(text=f"{self.bot.user.name} Ticket", icon_url=self.bot.user.avatar.url))
            await asyncio.sleep(10)
        else:
            x = catmenuview(self.ctx, "opencategory", "Select the Open ticket's category")
            embed = discord.Embed(title=f"{self.name} Ticket panel setup", description="Select a category for open tickets\nIf a category is not listed below just start typing it's name in select menu box it will be shown as a option", color=0x2f3136).set_footer(text=f"{self.bot.user.name} Ticket", icon_url=self.bot.user.avatar.url)
            await interaction.edit_original_response(embed=embed, view=x)
            await x.wait()
        emb = await configdata(interaction.guild)
        await interaction.edit_original_response(embed=emb, view=self)
    ...
    async def embmsg(self, interaction, button):
        embed = discord.Embed(title=f"{self.name} Ticket panel setup", description=f"What should be the Embed message for {self.name}'s Panel", color=0x2f3136).set_footer(text=f"{self.bot.user.name} Ticket", icon_url=self.bot.user.avatar.url)
        def check(message):
    ...
    async def done(self, interaction, button):
        ...
        with sqlite3.connect('data/database.sqlite3') as db:
            ...
            cursor.execute(query, val)
            m_db = cursor.fetchone()
        emb = discord.Embed(title=f"{m_db['name']} Panel", description=m_db['msg'], color=0x2f3136).set_footer(text=f"{str(self.bot.user.name)} Ticket System", icon_url=self.bot.user.avatar.url)
...
class panelview(discord.ui.View):
    ...
    async def channel(self, interaction: discord.Interaction, button):
        ...
        embed = discord.Embed(title=f"{self.name} Ticket panel setup", description="Where should I setup the ticket panel?\nIf a channel is not listed below just start typing it's name in select menu box it will be shown as a option", color=0x2f3136).set_footer(text=f"{self.bot.user.name} Ticket", icon_url=self.bot.user.avatar.url)
        await interaction.edit_original_response(embed=embed, view=x)
        await x.wait()
    ...
    async def open(self, interaction, button):
        ...
        if len(xd) == 0:
            ...
            x = catmenuview(self.ctx, "opencategory", "Select the Open ticket's category")
            embed = discord.Embed(title=f"{self.name} Ticket panel setup", description="Select a category for open tickets\nIf a category is not listed below just start typing it in select menu box it's name will be shown as a option", color=0x2f3136).set_footer(text=f"{self.bot.user.name} Ticket", icon_url=self.bot.user.avatar.url)
            await interaction.edit_original_response(embed=embed, view=x)
            await x.wait()
        emb = await configdata(interaction.guild)
        await interaction.edit_original_response(embed=emb, view=self)
...
class ticket(commands.Cog):
    ...
    async def ticket(self, ctx):
        ...
        if prefix == f"<@{self.bot.user.id}> ":
            prefix = "@Redox "
        anay = discord.utils.get(self.bot.users, id=1126351590064930847)
        ls = ["ticket","ticket setup","ticket delete", "ticket edit", "ticket info", "ticket reopen"]
        des = ""
        for i in sorted(ls):
            cmd = self.bot.get_command(i)
            des += f"`{prefix}{i}`\n{cmd.description}\n\n"
        listem = discord.Embed(title=f"Ticket Commands", colour=0x2f3136,
    ...
    async def setup(self, ctx: commands.Context):
        if ctx.guild.owner.id == ctx.author.id:
            ...
            if ctx.author.top_role.position <= ctx.guild.me.top_role.position and ctx.author.id not in  [926831289649201213]:
                em = discord.Embed(description=f"<a:crossss:1174609979932684328> | You must Have Higher Role than Bot To run This Command", color=0xff0000)
        ...
        if m_db is None:
            ...
            return await ctx.send(embed=discord.Embed(description=f"<a:crossss:1174609979932684328> | You can create only one panel at the moment. ", color=0x2f3136).set_footer(text=self.bot.user.name, icon_url=self.bot.user.avatar.url))
        db.commit()
        cursor.close()
        db.close()
    ...
    async def delete(self, ctx: commands.Context):
        ...
        with sqlite3.connect('data/database.sqlite3') as db:
            ...
            m_db = cursor.fetchone()
        if m_db is None:
            return await ctx.send(embed=discord.Embed(description=f"<a:crossss:1174609979932684328> | No ticket panel found with the name `{name}`", color=0x2f3136).set_footer(text=self.bot.user.name, icon_url=self.bot.user.avatar.url))
        try:
            c = self.bot.get_channel(m_db["channel_id"])
            m = await c.fetch_message(m_db['msg_id'])
        ...
        return await ctx.send(embed=discord.Embed(description=f"<a:tickkk:1223594613961523281> | Successfully deleted ticket panel with name `{name}`", color=0x2f3136).set_footer(text=self.bot.user.name, icon_url=self.bot.user.avatar.url))
