...
PICKUP_LINES = json.loads(Path("data/json/pikup.json").read_text("utf8"))
...
class Fun(commands.Cog):
    ...
    async def hug(self, ctx, user: discord.Member = None):
        ...
        embed = discord.Embed(
            title=
            f'{ctx.author.name} hugs {f"themselves. 😔" if user is None else f"{user.name} <:Redox_bruh:1138863953171189800>"}',
            color=0x00FFCA,
    ...
    async def meme(self, ctx):
        embed = discord.Embed(title="""Take some memes""", color=0x00FFCA)
        async with aiohttp.ClientSession() as cs:
            async with cs.get(
    ...
    async def cat(self, ctx):
        embed = discord.Embed(title="""Here's a cat""", color=0x00FFCA)
        async with aiohttp.ClientSession() as cs:
