...
class Media(commands.Cog):
    ...
    async def on_ready(self):
      with open("data/json/media.json", "r", encoding="utf-8") as f:
        ok = json.load(f)
      for guild in self.client.guilds:
        if not str(guild.id) in ok:
          ok[str(guild.id)] = {"channel": []}
      with open("data/json/media.json", "w", encoding="utf-8") as f:
    ...
    async def setup(self, ctx, *, channel: discord.TextChannel):
        ...
        await ctx.send(f"<a:tickkk:1130730211382661171> | Successfully added {channel.mention} in media only channels.")
    ...
    async def remove(self, ctx, *, channel: discord.TextChannel):
        ...
        await ctx.send(f"<a:tickkk:1130730211382661171> | Successfully removed {channel.mention} from media only channels.")
    ...
    async def reset(self, ctx):
        ...
        await ctx.send("<a:tickkk:1130730211382661171> | Successfully removed all the media only channels.")
