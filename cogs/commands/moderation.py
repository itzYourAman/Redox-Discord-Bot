...
class Moderation(commands.Cog):
  ...
  async def unlockall(self, ctx, server: discord.Guild = None, *, reason=None):
    hacker = discord.Embed(
      color=0x0d0d13,
      description=
      "<a:tickkk:1130730211382661171> | Unlocking all channels in few seconds .",
  ...
  async def lockall(self, ctx, server: discord.Guild = None, *, reason=None):
    hacker = discord.Embed(
      color=0x00FFCA,
      description=
      "<a:tickkk:1130730211382661171> | Locking all channels in few seconds .",
      timestamp=ctx.message.created_at)
    hacker.set_author(name=f"{ctx.author.name}",
                      icon_url=f"{ctx.author.avatar}")
    await ctx.reply(embed=hacker)
 ...
 # @ignore_check()
  ...
  async def hackban(self, ctx, userid, *, reason=None):
    ...
    try:
      ...
      await ctx.reply(embed=discord.Embed(
        description=
        f"<a:tickkk:1130730211382661171> | Successfully hackbanned {userid} for {reason}",
        color=0x00FFCA))
    except:
      await ctx.reply(embed=discord.Embed(
  ...
  async def give(
    ...
    if guild.me.top_role >= ctx.author.top_role:
      embed = discord.Embed(
        color=0x00FFCA,
        description=
        "<a:crossss:1131829269509709875> | Your top role should be above my top role."
      )
      await ctx.send(embed=embed)
      return
    if role.position >= guild.me.top_role.position:
      await ctx.send(
        f"**<a:crossss:1131829269509709875> | The position of `{role}` is above my top role. **"
      )
      return
    else:
  ...
  async def _hideall(self, ctx):
    hacker = discord.Embed(
      color=0x00FFCA,
      description=
      "<a:tickkk:1130730211382661171> | Hiding all channels in few seconds .",
      timestamp=ctx.message.created_at)
    hacker.set_author(name=f"{ctx.author.name}",
                      icon_url=f"{ctx.author.avatar}")
  ...
  async def _unhideall(self, ctx):
    hacker = discord.Embed(
      color=0x00FFCA,
      description=
      f"<a:tickkk:1130730211382661171> | Unhiding all channels in few seconds .",
      timestamp=ctx.message.created_at)
    hacker.set_author(name=f"{ctx.author.name}",
                      icon_url=f"{ctx.author.avatar}")
    #hacker.set_thumbnail(url =f"{ctx.author.avatar}")
  ...
  async def _hide(self, ctx, channel: discord.TextChannel = None):
    ...
    hacker = discord.Embed(
      color=0x00FFCA,
      description=
      f"<a:tickkk:1130730211382661171> | Succefully Hidden {channel.mention} .",
      timestamp=ctx.message.created_at)
    hacker.set_author(name=f"{ctx.author.name}",
                      icon_url=f"{ctx.author.avatar}")
    hacker.set_thumbnail(url=f"{ctx.author.avatar}")
  ...
  async def _unhide(self, ctx, channel: discord.TextChannel = None):
    ...
    hacker = discord.Embed(
      color=0x00FFCA,
      description=
      f"<a:tickkk:1130730211382661171> | Succefully Unhidden {channel.mention} .",
      timestamp=ctx.message.created_at)
    hacker.set_author(name=f"{ctx.author.name}",
                      icon_url=f"{ctx.author.avatar}")
    hacker.set_thumbnail(url=f"{ctx.author.avatar}")
...
#  @ignore_check()
  ...
  async def _softban(self,
    ...
    hacker = discord.Embed(
      color=0x00FFCA,
      description=
      f"<a:tickkk:1130730211382661171> | Sucessfully soft-banned {member}.",
      timestamp=ctx.message.created_at)
 ...
 # @ignore_check()
    ...
    if member.is_timed_out():
      try:
        ...
        hacker5 = discord.Embed(
          color=0x00FFCA,
          description=
          f"<a:tickkk:1130730211382661171> | Successfully Unmuted {member.name}",
          timestamp=ctx.message.created_at)
        ...
        hacker = discord.Embed(
          color=0x00FFCA,
          description=
          "<a:crossss:1131829269509709875> | Unable to Remove Timeout:\n```py\n{}```"
          .format(e),
          timestamp=ctx.message.created_at)
        await ctx.send(embed=hacker)
    else:
      hacker1 = discord.Embed(
        color=0x00FFCA,
        description="<a:crossss:1131829269509709875> | {} Is Not Muted".
  ...
  async def _kick(self,
    ...
    if ctx.author.top_role.position > member.top_role.position or member == ctx.guild.owner:
      ...
      hacker = discord.Embed(
        color=0x00FFCA,
        description=
        f"<a:crossss:1131829269509709875> | {member.display_name} has been kicked from this guild, for: {reason}",
        timestamp=ctx.message.created_at)
      #hacker.set_footer(text=f"Made With 💖 By ~ GODFATHER ! </>#9417",icon_url= "https://media.discordapp.net/attachments/1066637418557624340/1068042323088379914/2491-couple-matching-1-2.gif")
      hacker.set_author(name=f"{ctx.author.name}",
                        icon_url=f"{ctx.author.avatar}")
    ...
    if not ctx.author.top_role.position > member.top_role.position and ctx.author != ctx.guild.owner:
      await ctx.send(
        "*<a:crossss:1131829269509709875> | You cannot kick someone with a higher role than you!*"
      )
  ...
  async def _warn(self,
    ...
    hacker = discord.Embed(
      color=0x00FFCA,
      description=
      f"<a:tickkk:1130730211382661171> | {member.display_name} has been warned for: {reason}",
      timestamp=ctx.message.created_at)
  ...
  async def _ban(self,
    ...
    if ctx.author.top_role.position > member.top_role.position or ctx.author == ctx.guild.owner:
      ...
      hacker = discord.Embed(
        color=0x00FFE4,
        description=
        f"<a:tickkk:1130730211382661171> | {member.display_name} has been successfully banned for the reason: `{reason}`",
        timestamp=ctx.message.created_at)
      hacker.set_author(name=f"{ctx.author.name}",
                        icon_url=f"{ctx.author.avatar}")
      hacker.set_thumbnail(url=f"{ctx.author.avatar}")
    ...
    if not ctx.author.top_role.position > member.top_role.position and ctx.author != ctx.guild.owner:
      embed = discord.Embed(
        color=0x00FFE4,
        description=
        f"*<a:crossss:1131829269509709875> | You cannot ban someone with a higher role than you.*",
        timestamp=ctx.message.created_at)
      await ctx.send(embed=embed)
  ...
  async def _unban(self, ctx: commands.Context, id: int):
    ...
    hacker = discord.Embed(
      color=0x00FFCA,
      description=
      f"<a:tickkk:1130730211382661171> | {user.name} has been successfully unbanned",
      timestamp=ctx.message.created_at)
    #hacker.set_footer(text=f"Made With 💖 By ~ Hacker_xD#0001",icon_url= "https://media.discordapp.net/attachments/1004709672588161044/1005073754889654343/a_9a2d97cca8cf934ac4b3624051ed9baf.gif")
    hacker.set_author(name=f"{ctx.author.name}",
                      icon_url=f"{ctx.author.avatar}")
  ...
  async def clone(self, ctx: commands.Context, channel: discord.TextChannel):
    ...
    hacker = discord.Embed(
      color=0x0d0d13,
      description=
      f"<a:tickkk:1130730211382661171> | {channel.name} has been successfully cloned",
      timestamp=ctx.message.created_at)
    #hacker.set_footer(text=f"Made With 💖 By ~ Hacker_xD#0001",icon_url= "https://media.discordapp.net/attachments/1004709672588161044/1005073754889654343/a_9a2d97cca8cf934ac4b3624051ed9baf.gif")
    hacker.set_author(name=f"{ctx.author.name}",
                      icon_url=f"{ctx.author.avatar}")
  ...
  async def changenickname(self, ctx: commands.Context, member: discord.Member,
    ...
    hacker = discord.Embed(
      color=0x0d0d13,
      description=
      f"<a:tickkk:1130730211382661171> | Successfully changed nickname of {member.name}",
      timestamp=ctx.message.created_at)
    hacker.set_author(name=f"{ctx.author.name}",
                      icon_url=f"{ctx.author.avatar}")
    hacker.set_thumbnail(url=f"{ctx.author.avatar}")
  ...
  async def _lock(self,
    ...
    try:
      ...
      await ctx.send(embed=discord.Embed(
        title="Redox | Lockdown",
        description=
        "<a:tickkk:1130730211382661171> | Successfully locked **%s**" %
        (channel.mention),
        color=0x00FFCA))
    except:
      await ctx.send(
        embed=discord.Embed(title="Redox | Lockdown",
  ...
  async def _unlock(self,
    ...
    try:
      await channel.set_permissions(
        ...
        reason=reason)
      await ctx.send(embed=discord.Embed(
        title="Redox | Unlockdown",
        description=
        "<a:tickkk:1130730211382661171> | Successfully unlocked **%s**" %
        (channel.mention),
        color=0x00FFCA))
    except:
      await ctx.send(
        embed=discord.Embed(title="Redox | Unlockdown",
...
**Description:** {pack.description}
  ...
  async def kljl(self, ctx, role: discord.Role):
    ...
    await ctx.send(
      "<a:tickkk:1130730211382661171> | Removing roles from all humans")
    for h in humans:
      await h.remove_roles(role)
    await ctx.reply(
      '<a:tickkk:1130730211382661171> | Removed mentioned role from all members'
    )
  ...
  async def kjjk(self, ctx, role: discord.Role):
    ...
    await ctx.send(
      "<a:tickkk:1130730211382661171> | Removing roles from all")
    for h in humans:
      await h.remove_roles(role)
    await ctx.reply(
      '<a:tickkk:1130730211382661171> | Removed mentioned role from all')
  ...
  async def lkklkl(self, ctx, role: discord.Role):
    ...
    await ctx.send(
      "<a:tickkk:1130730211382661171> | Removing roles to all humans & bots"
    )
    for h in humans:
      await h.remove_roles(role)
    await ctx.reply(
      '<a:tickkk:1130730211382661171> | Removed mentioned role from all bots'
    )
