...
async def create_timer(id, name, exp, cre, auth, c):
  with open("data/json/remind.json", "r", encoding="utf-8") as f:
    datab = json.load(f)
  data = {'name': name, "id": id, "guild": c.guild.id, "created": cre, "expire": exp, "author": auth.id, "channel": c.id}
  datab[str(id)] = data
  with open("data/json/remind.json", "w", encoding="utf-8") as f:
...
class Reminder(commands.Cog):
  ...
  @_ling.command(name="start", aliases=["create", "set"],help="Creates a reminder.")
  @blacklist_check()
  @ignore_check()
  ...
  @_ling.command(name="delete", aliases=["remove","close","rmv"],help="Deletes a reminder.")
  @blacklist_check()
  @ignore_check()
  async def _lingxd(self,ctx,msg_id):
    ...
    await ctx.send("<a:tickkk:1130730211382661171> | Successfully deleted reminder.")
  @commands.Cog.listener()
