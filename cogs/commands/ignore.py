    ...
    async def channel_add(self, ctx: Context, channel: discord.TextChannel):
        ...
        if ctx.author == ctx.guild.owner or ctx.author.top_role.position > ctx.guild.me.top_role.position:
            if str(channel.id) in ch:
                embed = discord.Embed(
                        description=
                        f"<a:crossss:1131829269509709875> | {channel.mention} is already in ignore channel list .",
                        color=self.color)
                ...
                hacker4 = discord.Embed(
                        color=self.color,
                        description=
                        f"<a:tickkk:1130730211382661171> | Successfully added {channel.mention} to ignore channel list .")
        ...
        if ctx.author == ctx.guild.owner or ctx.author.top_role.position > ctx.guild.me.top_role.position:   
            if len(ch) == 0:
                hacker = discord.Embed(
                    color=self.color,
                    description=
                    "<a:crossss:1131829269509709875> | This server dont have any ignore channel setupped yet .")
                ...
                if str(channel.id) not in ch:   
                    hacker1 = discord.Embed(
                        color=self.color,
                        description=
                        "<a:crossss:1131829269509709875> | This channel is not in the ignore channels list .")
                    ...
                    hacker3 = discord.Embed(
                        color=self.color,
                        description=
                        f"<a:tickkk:1130730211382661171> | Successfully removed {channel.mention} from ignore channel list .")
    ...
    async def user_add(self, ctx: Context, user: discord.User):
        ...
        if ctx.author == ctx.guild.owner or ctx.author.top_role.position > ctx.guild.me.top_role.position:
            if str(user.id) in ch:
                embed = discord.Embed(
                        description=
                        f"<a:crossss:1131829269509709875> | {user.mention} is already in ignore users list .",
                        color=self.color)
                await ctx.reply(embed=embed, mention_author=False)
            else:
                ...
                hacker4 = discord.Embed(
                        color=self.color,
                        description=
                        f"<a:tickkk:1130730211382661171> | Successfully added {user.mention} to ignore users list .")
                hacker4.set_author(name=ctx.author,
                                       icon_url=ctx.author.avatar.url if ctx.author.avatar else ctx.author.default_avatar.url)
                await ctx.send(embed=hacker4)
        else:
        ...
        if ctx.author == ctx.guild.owner or ctx.author.top_role.position > ctx.guild.me.top_role.position:   
            if len(ch) == 0:
                hacker = discord.Embed(
                    color=self.color,
                    description=
                    "<a:crossss:1131829269509709875> | This server dont have any ignore users setupped yet .")
                hacker.set_author(name=ctx.author,
                                       icon_url=ctx.author.avatar.url if ctx.author.avatar else ctx.author.default_avatar.url)
                await ctx.send(embed=hacker)   
            else:
                if str(user.id) not in ch:   
                    hacker1 = discord.Embed(
                        color=self.color,
                        description=
                        f"<a:crossss:1131829269509709875> | {user.mention} is not in the ignore users list .")
                    hacker1.set_author(name=ctx.author,
                                       icon_url=ctx.author.avatar.url if ctx.author.avatar else ctx.author.default_avatar.url)
                    await ctx.send(embed=hacker1) 
                else:
                    ...
                    hacker3 = discord.Embed(
                        color=self.color,
                        description=
                        f"<a:tickkk:1130730211382661171> | Successfully removed {user.mention} from ignore users list .")
                    hacker3.set_author(name=ctx.author,
                                       icon_url=ctx.author.avatar.url if ctx.author.avatar else ctx.author.default_avatar.url)
                    await ctx.send(embed=hacker3)
        else:
    ...
    async def buser_add(self, ctx: Context, user: discord.User):
        ...
        if ctx.author == ctx.guild.owner or ctx.author.top_role.position > ctx.guild.me.top_role.position:
            if str(user.id) in ch:
                embed = discord.Embed(
                        description=
                        f"<a:crossss:1131829269509709875> | {user.mention} is already in bypass users list .",
                        color=self.color)
                await ctx.reply(embed=embed, mention_author=False)
            else:
                ch.append(str(user.id))
                ...
                hacker4 = discord.Embed(
                        color=self.color,
                        description=
                        f"<a:tickkk:1130730211382661171> | Successfully added {user.mention} to bypass users list .")
                hacker4.set_author(name=ctx.author,
                                       icon_url=ctx.author.avatar.url if ctx.author.avatar else ctx.author.default_avatar.url)
                await ctx.send(embed=hacker4)
        else:
        ...
        if ctx.author == ctx.guild.owner or ctx.author.top_role.position > ctx.guild.me.top_role.position:   
            if len(ch) == 0:
                hacker = discord.Embed(
                    color=self.color,
                    description=
                    "<a:crossss:1131829269509709875> | This server dont have any bypass users setupped yet .")
                hacker.set_author(name=ctx.author,
                                       icon_url=ctx.author.avatar.url if ctx.author.avatar else ctx.author.default_avatar.url)
                await ctx.send(embed=hacker)   
            else:
                if str(user.id) not in ch:   
                    hacker1 = discord.Embed(
                        color=self.color,
                        description=
                        f"<a:crossss:1131829269509709875> | {user.mention} is not in the bypass users list .")
                    hacker1.set_author(name=ctx.author,
                                       icon_url=ctx.author.avatar.url if ctx.author.avatar else ctx.author.default_avatar.url)
                    await ctx.send(embed=hacker1) 
                else:
                    ...
                    hacker3 = discord.Embed(
                        color=self.color,
                        description=
                        f"<a:tickkk:1130730211382661171> | Successfully removed {user.mention} from bypass users list .")
                    hacker3.set_author(name=ctx.author,
                                       icon_url=ctx.author.avatar.url if ctx.author.avatar else ctx.author.default_avatar.url)
                    await ctx.send(embed=hacker3)
        else:
    ...
    async def iuser_show(self, ctx):
        ...
        if len(ch) == 0:
            hacker = discord.Embed(
                ...
                description=
                "<a:crossss:1131829269509709875> | There aren\'t any ignored users for this server"
            )
    ...
    async def buser_show(self, ctx):
        ...
        if len(ch) == 0:
            hacker = discord.Embed(
                ...
                title=f"{self.client.user.name}",
                description=
                "<a:crossss:1131829269509709875> | There aren\'t any ignore bypass users for this server"
            )
            await ctx.reply(embed=hacker, mention_author=False)
        else:
            entries = [
