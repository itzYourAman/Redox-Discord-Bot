...
class Automod(Cog):
    ...
    async def _antiraid(self, ctx: commands.Context):
        ...
        spam = data["antiSpam"]
        link = data["antiLink"]
        embed = discord.Embed(title="Redox | Automod Commands",
                              color=0x00FFCA)
        embed.add_field(
            name="<a:sword:1138827398813589554> antispam on/off",
            value=f"Enables/Disables antispam feature\nCurrently Its {spam}",
            inline=False)
        embed.add_field(
            name="<a:sword:1138827398813589554> antilink on/off",
    ...
    async def _antispam(self, ctx: commands.Context, type: str):
        ...
        if ctx.author == ctx.guild.owner or ctx.guild.me.top_role <= ctx.author.top_role:
            if onOroff == "on":
                if data["antiSpam"] is True:
                    hacker = discord.Embed(
                        description=
                        f"<a:crossss:1131829269509709875> | Anti-Spam is already enabled for **`{ctx.guild.name}`**",
                        color=0x00FFCA)
                    ...
                    hacker1 = discord.Embed(
                        ...
                        description=
                        f"<a:tickkk:1130730211382661171> | Successfully enabled anti-spam for **`{ctx.guild.name}`**",
                        color=0x00FFCA)
                ...
                hacker2 = discord.Embed(
                    description=
                    f"<a:tickkk:1130730211382661171> | Successfully disabled anti-spam for **`{ctx.guild.name}`**",
                    color=0x00FFCA)
                await ctx.reply(embed=hacker2, mention_author=False)
            else:
                hacker3 = discord.Embed(
                    description=
                    f"<a:crossss:1131829269509709875> | Invalid Type.\nIt Should Be On/Off",
                    color=0x00FFCA)
            ...
            hacker5 = discord.Embed(
                color=0x00FFCA,
                title="Redox Security",
                description=
                f"<a:crossss:1131829269509709875> | Only owner of the server can run this command"
            )
        ...
        if ctx.author == ctx.guild.owner or ctx.guild.me.top_role <= ctx.author.top_role:
            if onOroff == "on":
                if data["antiLink"] is True:
                    hacker = discord.Embed(
                        description=
                        f"<a:crossss:1131829269509709875> | Anti-link is already enabled for **`{ctx.guild.name}`**",
                        color=0x00FFCA)
                    await ctx.reply(embed=hacker, mention_author=False)
                else:
                    data["antiLink"] = True
                    ...
                    hacker1 = discord.Embed(
                        description=
                        f"<a:tickkk:1130730211382661171> | Successfully enabled anti-link for **`{ctx.guild.name}`**",
                        color=0x00FFCA)
                    await ctx.reply(embed=hacker1, mention_author=False)
                ...
                hacker2 = discord.Embed(
                    description=
                    f"<a:tickkk:1130730211382661171> | Successfully disabled anti-link for **`{ctx.guild.name}`**",
                    color=0x00FFCA)
                await ctx.reply(embed=hacker2, mention_author=False)
            else:
                hacker3 = discord.Embed(
