...
class Vanityroles(commands.Cog):
    ...
    async def _setup(self, ctx, vanity, role: discord.Role,
        ...
        with open("data/json/vanityroles.json", "r", encoding="utf-8") as f:
            idk = json.load(f)
            if ctx.author == ctx.guild.owner or ctx.guild.me.top_role <= ctx.author.top_role:
                if role.permissions.administrator or role.permissions.ban_members or role.permissions.kick_members:
                    embed1 = discord.Embed(
                        description=
                        "<a:crossss:1131829269509709875> | Please Select Role That Dont Have Any Type Of Dangerous Permissions.",
                        color=0x00FFCA)
                    ...
                    embed.add_field(
                        name="<:invitesss:1125056655567101972> Vanity",
                        value=f"{vanity}",
                        inline=False)
                    embed.add_field(
                        name="<:role:1125085061658706000> Role",
                        value=f"{role.mention}",
                        inline=False)
                    embed.add_field(
                        name="<:icons_channel:1125103333867266279> Channel",
                        value=f"{channel.mention}",
                        inline=False)
                    await ctx.send(embed=embed)
                    with open("data/json/vanityroles.json", "w", encoding="utf-8") as f:
        ...
        with open("data/json/vanityroles.json", "r", encoding="utf-8") as f:
            ...
            if ctx.author == ctx.guild.owner or ctx.guild.me.top_role <= ctx.author.top_role:
                if str(ctx.guild.id) not in jnl:
                    embed1 = discord.Embed(
                        description=
                        "<a:crossss:1131829269509709875> | This server don't have any vanity roles setupped yet .",
                        color=0x00FFCA)
    ...
    async def config(self, ctx):
        ...
        if str(ctx.guild.id) not in jnl:
            ...
            embed.add_field(name="<:invitesss:1125056655567101972> Vanity",
                            value=f"{vanity}",
                            inline=False)
            embed.add_field(name="<:role:1125085061658706000> Role",
                            value=f"{randirole.mention}",
                            inline=False)
            embed.add_field(name="<:icons_channel:1125103333867266279> Channel",
    ...
    async def on_presence_update(self, before, after):
        ...
        if vanity in str(after.activity).lower() and role not in after.roles:
            ...
            embed = discord.Embed(
                color=0x030404,
                description=f"{after.mention} Thanks for representing Vanity {vanity} in your status ❤️",
        ...
        # Vanity role removal
            ...
            embed = discord.Embed(
                color=0x030404,
                description=f"{after.mention} Removed Vanity {vanity} From Their Status ❤️",
                timestamp=discord.utils.utcnow()
            )
            embed.set_author(name=after.name, icon_url=after.display_avatar.url)
            embed.set_footer(text="Thanks For Choosing This Server.")
