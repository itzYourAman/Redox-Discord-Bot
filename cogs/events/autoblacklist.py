...
class AutoBlacklist(Cog):
    ...
    async def on_message(self, message):
      ...
      Redox = '<@1138518171670810634>'
      retry = bucket.update_rate_limit()

      if retry:
        if message.content == Redox or message.content == "<@!1138518171670810634>":
          add_user_to_blacklist(message.author.id)
          embed = discord.Embed(description="**Successfully Blacklisted {} For Spam Mentioning Me!**".format(message.author.mention),color=0x00FFCA)
    ...
    async def on_command(self, ctx):
      ...
      if retry:
        ...
        embed = discord.Embed(description="**Successfully Blacklisted {} For Spamming My Commands!**".format(ctx.author.mention),color=0x00FFCA)
        embed.set_thumbnail( url=self.client.user.display_avatar.url)
